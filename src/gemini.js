...
async function requestGeminiTurn(userText, context, retryReason = "") {
    ...
    const { model, sessionContext, checkpoints, lastUserMessage, inClosingPhase = false } = context;
    ...
    try {
        res = await fetch("/api/gemini", {
            ...
            body: JSON.stringify({
                model,
                systemPrompt: buildSystemPrompt(sessionContext, checkpoints, retryReason, { inClosingPhase }),
